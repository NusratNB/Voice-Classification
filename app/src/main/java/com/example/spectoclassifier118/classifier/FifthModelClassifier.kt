...
class FifthModelClassifier {
    ...
    fun makeInference(activity: AssetManager, data: FloatArray, inpAudioLength: Int, modName: String): Array<FloatArray> {
        ...
        if (locNumPredictions == 0){
            ...
            Log.d("fifthMod tempNumFrames", tempNumFrames.toString())
            Log.d("fifthMod localBatchSize", localBatchSize.toString())
            Log.d("fifthMod before locNumPredictions", locNumPredictions.toString())
            locNumPredictions = 1
            Log.d("fifthMod after locNumPredictions", locNumPredictions.toString())
